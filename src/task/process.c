...
static struct process* processes[RAOS_MAX_PROCESSES] = {};
...
struct process* process_get(int process_id) {
    if (process_id < 0 || process_id >= RAOS_MAX_PROCESSES) {
        return NULL;
    }
...
static int process_find_free_allocation_index(struct process* process) {
    int res = -ENOMEM;
    for (int i = 0; i < RAOS_MAX_PROGRAM_ALLOCATIONS; i++) {
...
void* process_malloc(struct process* process, size_t size) {
    ...
    int res = paging_map_to(process->task->page_directory, ptr, ptr,
                            ...
                            PAGING_IS_WRITABLE | PAGING_IS_PRESENT
...
void process_switch_to_any() {
    for (int i = 0; i < RAOS_MAX_PROCESSES; i++) {
        if (processes[i]) {
...
/**
 * @brief Load elf or binary data into the process
 *
 * @param filename
 * @param process
 * @return int
...
int process_map_binary(struct process* process) {
    int res = 0;
    paging_map_to(
        process->task->page_directory, (void*)RAOS_PROGRAM_VIRTUAL_ADDRESS,
...
int process_map_memory(struct process* process) {
    ...
    paging_map_to(
        process->task->page_directory,
        (void*)RAOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack,
        paging_align_address(process->stack + RAOS_USER_PROGRAM_STACK_SIZE),
        PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITABLE);
out:
    return res;
}
...
/**
 * @brief Get a free slot in the process array
 *
 * @return int
 */
...
/**
 * @brief Load the file into available process.
 *
 * @param filename
 * @param process
 * @return int
...
/**
 * @brief Load the file into process
 *
 * @param filename
 * @param process
 * @param process_slot a index value in processes array
 * @return int
 */
int process_load_for_slot(const char* filename, struct process** process,
    ...
    // === Prepare stack space
    program_stack_ptr = kzalloc(RAOS_USER_PROGRAM_STACK_SIZE);
    if (!program_stack_ptr) {
        res = -ENOMEM;
        goto out;
    }

    // === Fill the _process structure
    strncpy(_process->filename, filename, sizeof(_process->filename));
    _process->stack = program_stack_ptr;
    _process->id    = process_slot;

    // === create a task for process
